...
public class HttpRequestUtil {
	...
	/**
	 * Returns URLConnection that 'posts' the given JSON to the given
	 ...
	 * @throws Exception
	 */
	public static URLConnection post(String url, String encodedCredentials,
			String jsonPayloadObject, Charset charset, ProxyConfig proxy,
			TrustStoreConfig customTrustStore) throws Exception {
		...
		if (customTrustStore != null
				&& customTrustStore.getTrustStorePath() != null
				&& conn instanceof HttpsURLConnection) {
			KeyStore trustStore = TrustStoreManagerService
					.getInstance()
					.getTrustStoreManager()
					.loadTrustStore(customTrustStore.getTrustStorePath(),
							customTrustStore.getTrustStoreType(),
	...
	private static URLConnection getConnection(String url,
			final ProxyConfig proxy) throws IOException {
		...
		if (proxy != null && proxy.getProxyUser() != null) {
			Authenticator.setDefault(new Authenticator() {
				...
				protected PasswordAuthentication getPasswordAuthentication() {
					return new PasswordAuthentication(proxy.getProxyUser(),
							proxy.getProxyPassword().toCharArray());
				}
		...
		if (proxy != null) {
			Proxy thisProxy = new Proxy(proxy.getProxyType(),
					new InetSocketAddress(proxy.getProxyHost(),
							proxy.getProxyPort()));
