...
public class DefaultPushSender implements PushSender {
	...
	private static final Logger logger = Logger
			.getLogger(DefaultPushSender.class.getName());
	 ...
	 *
	 * @param location
	 *            of the push configuration file
	 * @return a {@link Builder} instance
	 */
	...
	public static class Builder {
		...
		private Builder(String rootServerURL) {
			...
			pushConfiguration
					.setServerUrl(!rootServerURL.endsWith("/") ? rootServerURL + '/'
							: rootServerURL);
		}
		 ...
		 *            The trustStore type. If null the default type iss used.
		 * @param trustStorePassword
		 *            The trustStore password.
		 * @return the current {@link Builder} instance
		 */
		public Builder customTrustStore(String trustStorePath,
				String trustStoreType, String trustStorePassword) {
			customTrustStore = new TrustStoreConfig(trustStorePath,
	...
	public void send(UnifiedMessage unifiedMessage,
		...
		submitPayload(buildUrl(), jsonString,
				...
				pushConfiguration.getMasterSecret(), callback,
	 ...
	 * 
	 * @param url
	 *            the URL to use for the HTTP POST request.
	 * @param jsonPayloadObject
	 *            the JSON payload of the POST request
	 * @param pushApplicationId
	 *            the registered applications identifier.
	 * @param masterSecret
	 *            the master secret for the push server.
	 * @param callback
	 *            the {@link MessageResponseCallback} that will be called once
	 *            the POST request completes.
	 * @param redirectUrls
	 *            a list containing the previous redirectUrls, used to detect an
	 *            infinite loop
	 * @throws {@link org.jboss.aerogear.unifiedpush.exception.PushSenderHttpException}
	 *         when delivering push message to Unified Push Server fails.
	 * @throws {@link org.jboss.aerogear.unifiedpush.exception.PushSenderException}
	 *         when generic error during sending occurs, such as an infinite
	 *         redirect loop.
	 */
	private void submitPayload(String url, String jsonPayloadObject,
			String pushApplicationId, String masterSecret,
		...
		try {
			...
			final String encoded = Base64.encodeBytes(credentials
					.getBytes(UTF_8));

			// POST the payload to the UnifiedPush Server
			httpURLConnection = (HttpURLConnection) HttpRequestUtil.post(url,
					encoded, jsonPayloadObject, UTF_8, proxy, customTrustStore);
			...
			// if we got a redirect, let's extract the 'Location' header from
			// the response
			// and submit the payload again
			if (isRedirect(statusCode)) {
				String redirectURL = httpURLConnection
						.getHeaderField("Location");
				logger.log(Level.INFO, String.format(
						"Performing redirect to '%s'", redirectURL));
				// execute the 'redirect'
				submitPayload(redirectURL, jsonPayloadObject,
						pushApplicationId, masterSecret, callback, redirectUrls);
			} else if (statusCode >= 400) {
				// treating any 400/500 error codes an an exception to a sending
				// attempt:
